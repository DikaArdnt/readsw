...
export default async function serialize(hisoka, msg) {
   ...
   if (msg.key) {
      m.key = msg.key
      m.from = (m.key.remoteJid == "status@broadcast") ? jidNormalizedUser(m.key.participant) : jidNormalizedUser(m.key.remoteJid)
      ...
      if (m.msg?.contextInfo?.quotedMessage) {
         ...
         if (m.quoted.message) {
            ...
            m.quoted.key = {
               ...
               participant: m.msg?.contextInfo?.remoteJid?.endsWith("g.us") ? jidNormalizedUser(m.msg?.contextInfo?.participant) : undefined,
