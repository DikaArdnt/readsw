...
import baileys, { jidNormalizedUser, extractMessageContent, areJidsSameUser, downloadContentFromMessage, toBuffer } from "@whiskeysockets/baileys"
   ...
   const client = Object.defineProperties(hisoka, {
      getName: {
         async value(jid) {
            ...
            if (id.endsWith("g.us")) {
               let metadata = await hisoka.groupMetadata(id)
               return metadata.subject
            } else {
               let metadata = store.contacts[id]
               return (metadata.name || metadata.verifiedName || metadata.notify || parsePhoneNumber("+" + id.split("@")[0]).formatInternational())
            }
         }
      },
      ...
      sendContact: {
         async value(jid, number, quoted, options = {}) {
            ...
            for (let v of number) {
               v = v.replace(/\D+/g, "")
               list.push({
                  displayName: await hisoka.getName(v + "@s.whatsapp.net"),
      ...
      downloadMediaMessage: {
         async value(message, filename) {
            ...
            let media = await toBuffer(await downloadContentFromMessage(message, mime))
...
export default async function serialize(hisoka, msg, store) {
   ...
   if (m.isGroup) {
      m.metadata = store.groupMetadata[m.from] || await hisoka.groupMetadata(m.from)
